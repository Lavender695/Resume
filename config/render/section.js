...
export default [
  ...
  },
  {
    ...
    content: [
      ...
      p(`**寻星手札-笔记平台**`),
      p('*项目地址：* https://xunxing.vercel.app/'),
      p('*技术栈： Next.js + TypeScript + Plate.js + Yjs + Liveblocks + Supabase + Clerk + 火山引擎 + Vercel AI SDK*'),
      p('*项目介绍：* 一款支持多人实时协同编辑、AI 流式辅助生成、Local-first 架构的现代化全栈个人知识库应用。'),
      p('*主要工作：*'),
      p('- *全栈架构与安全鉴权：* 采用 Clerk 作为身份认证中心，集成 Supabase 数据库。配置 RLS（行级安全策略），实现用户数据的严格隔离与云端安全存储。'),
      p('- *CRDT 实时协同编辑：* 基于 Plate.js 搭建富文本编辑器，引入 Yjs（CRDT 算法）与 Liveblocks WebSocket 服务。'),
      p('- *Local-first 离线优先架构：* 结合 y-indexeddb 将协同文档的二进制状态持久化至浏览器本地，实现弱网/断网环境下的文档秒开与无缝编辑。'),
      p('- *AI 流式辅助创作（SSE）：* 接入 Vercel AI SDK 与火山引擎大模型，基于 SSE 实现 AI 内容的流式打字机输出。'),
